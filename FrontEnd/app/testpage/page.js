...
const Annotation = () => {
  ...
  const handleSkip = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const Src_Text = items[currentIndex]?.sourceText; // Adjust based on your data structure
      ...
      // 1) Send skip request to backend
      const res = await fetch(
        "https://anotationtoolbackend-production.up.railway.app/api/annotation/skip",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      // 2) Mark as saved (optional if you want to treat skipped items separately)
      const newSaved = [...savedIndices, currentIndex];
      setSavedIndices(newSaved);
      ...
      if (next !== null) {
        ...
        // 5) Save skip progress to backend
        await fetch(
          "https://anotationtoolbackend-production.up.railway.app/api/progress",
          {
            method: "POST",
      ...
      // 6) Clear comment and selections
      setComment("");
      clearSelections();
    } catch (err) {
            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  alert("Failed to fetch saved progress.");
                }
              }}
              className="btn btn-outline btn-info mt-2 text-sm"
            >
              Resume Last Annotated
            </button>
          </div>
