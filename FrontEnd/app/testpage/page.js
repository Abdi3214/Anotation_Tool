            ...
            <button
              onClick={async () => {
                ...
                try {
                  ...
                  const saved = await res.json();
                  const freshSavedIndices = saved
                    .map((a) =>
                      items.findIndex((item) => item.sourceText === a.Src_Issue)
                    )
                    .filter((idx) => idx >= 0);

                  const next = getNextMissing(freshSavedIndices, items.length);
                  if (next !== null) {
                    setCurrentIndex(next);
                    localStorage.setItem(
                      `annotationIndex_${taskId}`,
                      next.toString()
                    );
                  } else {
                    alert("🎉 You've completed all annotations!");
                  ...
                  alert("Failed to fetch latest annotation state.");
                }
              }}
              className="btn btn-secondary ml-4 px-6 py-2 text-sm sm:text-base font-semibold rounded-md dark:bg-gray-700 dark:hover:bg-gray-600"
            >
              Skip
            </button>
